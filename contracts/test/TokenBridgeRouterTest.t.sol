...
import {TokenBridgeRouter, RemoteBridge} from "../src/TokenBridgeRouter/TokenBridgeRouter.sol";
...
contract TokenBridgeRouterTest is Test {
    ...
    function testSetRelayerFee() public {
        vm.startPrank(owner);
        (
            uint256 primaryRelayerFeeStart,
            uint256 secondaryRelayerFeeStart
        ) = tokenBridgeRouter.getRelayerFeeBips();
        uint256 primaryRelayerFeeValue = 50;
        uint256 secondaryRelayerFeeValue = 20;
        tokenBridgeRouter.setRelayerFeesBips(
            primaryRelayerFeeValue,
            secondaryRelayerFeeValue
        );
        (
            uint256 primaryRelayerFeeEnd,
            uint256 secondaryRelayerFeeEnd
        ) = tokenBridgeRouter.getRelayerFeeBips();
        assert(
    ...
    function testGetHomeBridgeAfterRegister() public registerTokenBridge {
        assert(
            tokenBridgeRouter.getHomeBridge(address(erc20Token)) ==
                address(tokenHome)
        );
    }

    function testGetRemoteBridgeConfigAfterRegister()
        public
        registerTokenBridge
    {
        RemoteBridge memory remoteBridge = tokenBridgeRouter.getRemoteBridge(
            remoteChainID,
            address(erc20Token)
        );
        assert(
    ...
    function testEmitsOnRemoveRemoteTokenBridge() public {
        ...
        tokenBridgeRouter.removeRemoteTokenBridge(
            address(erc20Token),
            remoteChainID
        );
        vm.stopPrank();
