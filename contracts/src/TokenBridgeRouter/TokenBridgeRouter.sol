// (c) 2024, E36 Knots All rights reserved.
...
import {SendTokensInput} from "../interfaces/ITokenBridge.sol";
...
import "@openzeppelin/contracts@4.8.1/utils/Address.sol";
...
contract TokenBridgeRouter is Ownable, ReentrancyGuard {
     ...
     * @notice Address `0x0` is used for the native token
     */
    mapping(bytes32 => mapping(address => RemoteBridge))
        public tokenRemoteChainToRemoteBridge;
    ...
    function registerHomeTokenBridge(
        ...
        require(
            bridgeAddress.isContract(),
            "TeleporterBridgeRouter: bridgeAddress is not a contract"
        );
    ...
    function registerRemoteTokenBridge(
        ...
        emit RegisterRemoteTokenBridge(
            tokenAddress,
            remoteBridge,
            remoteChainID
        );
    ...
    function bridgeERC20(
        ...
        RemoteBridge memory remoteBridge = tokenRemoteChainToRemoteBridge[
            remoteChainID
        ][tokenAddress];
        require(
            bridgeHome != address(0),
            "TeleporterBridgeRouter: bridge not set for home + token"
        );
        require(
        ...
        uint256 primaryFeeAmount = SafeMath.div(
            SafeMath.mul(amount, primaryRelayerFeeBips),
            10_000
        );

        uint256 secondaryFeeAmount = SafeMath.div(
            SafeMath.mul(amount, secondaryRelayerFeeBips),
            10_000
        );
        ...
        IERC20TokenBridge(bridgeHome).send(input, bridgeAmount);
    ...
    function bridgeNative(
        ...
        require(
            bridgeHome != address(0),
            "TeleporterBridgeRouter: bridge not set for home"
        );
        require(
            remoteBridge.bridgeAddress != address(0),
        ...
        // Compute the fee amount
        uint256 primaryFeeAmount = SafeMath.div(
            SafeMath.mul(msg.value, primaryRelayerFeeBips),
            10_000
        );

        uint256 secondaryFeeAmount = SafeMath.div(
            SafeMath.mul(msg.value, secondaryRelayerFeeBips),
            10_000
        );
        ...
        INativeTokenBridge(bridgeHome).send{value: bridgeAmount}(input);
