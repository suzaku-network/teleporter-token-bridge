...
import {ProtocolRegistryEntry, TeleporterRegistry} from "@teleporter/upgrades/TeleporterRegistry.sol";
...
contract HelperConfig is Script {
    ...
    address private constant WARP_PRECOMPILE =
    ...
    function getNetworkConfig(
        ...
        protocolRegistryEntry.push(
            ProtocolRegistryEntry(1, address(teleporterMessenger))
        );
        TeleporterRegistry teleporterRegistry = new TeleporterRegistry(
            protocolRegistryEntry
        );

        ERC20Mock _erc20Token = new ERC20Mock("Mock", "MOCK", _owner, 0);
        WrappedNativeToken _wrappedToken = new WrappedNativeToken("WNTT"); // WNTT for Wrapped Native Token Test

        TokenBridgeRouter _tokenBridgeRouter = new TokenBridgeRouter(
            _primaryRelayerFeeBips,
            _secondaryRelayerFeeBips
        );

        ERC20TokenHome _erc20TokenHome = new ERC20TokenHome(
            address(teleporterRegistry),
            _owner,
            address(_erc20Token),
            18
        );
        NativeTokenHome _nativeTokenHome = new NativeTokenHome(
            address(teleporterRegistry),
            _owner,
